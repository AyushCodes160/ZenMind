...
function initNav(active) {
  ...
  sidebar.innerHTML = `
    ...
    <nav class="sidebar-nav">
      ...
      ${pages.map(p => `
        <a href="${p.href}" class="nav-link ${active === p.id ? 'active' : ''}">
          ...
          ${p.label}
          ${p.badge ? `<span class="nav-link-badge">${p.badge}</span>` : ''}
        </a>
      `).join('')}
    </nav>
    ...
    <div class="sidebar-bottom">
      ${streak > 0 ? `
        <div style="margin-bottom:10px;padding:10px 12px;background:oklch(24% 0.04 160);border-radius:12px;display:flex;align-items:center;gap:8px;">
          ...
          <div>
            <div style="font-size:13px;font-weight:600;color:white;">${streak}-day streak</div>
      ...
      <div class="sidebar-user">
        ...
        <div>
          ...
          <div class="user-plan">Calm Plan ✦</div>
        </div>
        <svg style="margin-left:auto" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="oklch(38% 0.03 160)" stroke-width="1.8">
